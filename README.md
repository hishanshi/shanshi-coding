...
| 前端改动完成，用户未要求浏览器验证 | 是否完成代码检查和不依赖浏览器的自动化测试，简述改动和必要的人工检查要点；界面交由人工验收，不主动操作浏览器、不等待或反复索要授权。 |
| 项目已有浏览器测试，但用户未要求浏览器验证 | 是否仅运行不依赖浏览器的检查与测试，说明未执行的浏览器测试，不因已有脚本而自动运行，也不将自检报为人工验收通过。 |
| 临时调试输出与原有诊断日志共存 | 是否清理本次临时内容并保护已有内容。 |
| 用户明确要求的浏览器验证完成或失败中止，且存在用户原有页面 | 本次新建且不再需要的页面、独立实例是否关闭，用户页面是否保留，截图等证据是否仍可查；未能收尾时是否说明。 |
