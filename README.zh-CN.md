...
`shanshi-coding` 是一个面向 AI 编码代理的 skill，用于当前轮次要求实际落地本地软件行为或工程控制面改动的任务，也用于人工审阅后明确要求的提交、推送、合并或发布操作。
...
- 更新运行、构建或 CI 配置和脚本
- 新增或调整测试并处理相关故障
- 人工审阅后明确要求的提交、推送、合并或发布

文档仅在工程变更确实需要时纳入。只要求方案、只读分析、评审，以及独立的内容或元数据维护时，不触发这个 skill。

## 它会约束什么

- 根据当前请求的交付物判断授权，而不是根据“修改”“调整”等局部字词判断。
- 方案、诊断、评审和比较默认只读；同一轮明确要求实施时除外。
